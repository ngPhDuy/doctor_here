  ...
  const fetchAppointment = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/appointment/detail?appointmentID= ${id}`
      );
      if (!response.ok) {
      ...
      const flatAppointment: Appointment = {
        ...
        ca_lam_viec_lam_viec_onl: data.Gio_hen.Ca_lam_viec_trong_tuan.lam_viec_onl,
        hinh_anh_bo_sung: data.Hinh_anh_bo_sung_cuoc_hen?.map((img: { url: string }) => img.url) || [],
      };      
  ...
  return (
    <div className="h-full bg-gray-50">
      ...
      <div className="flex items-center mb-4 bg-white rounded-lg shadow-md">
        <div className="p-3 cursor-pointer" onClick={() => navigate("/historyList")}>
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
      ...
      <div className="flex gap-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
              ...
              <input
                  className="w-full px-3 py-2 border rounded-lg bg-gray-100"
                  value={appointment.ca_lam_viec_lam_viec_onl ? "Online" : "Offline"}
                  disabled
              />
          </div>
          <div>
            ...
            <input
                className="w-full px-3 py-2 border rounded-lg bg-gray-100"
                value={`${formatTime(appointment.gio_hen_thoi_diem_bat_dau)} - ${formatTime(appointment.gio_hen_thoi_diem_ket_thuc)} ${formatDate(appointment.gio_hen_ngay_lam_viec)}`}
                disabled
            />
          </div>
        ...
        <div className="mt-2">
            ...
            <div className="w-full px-3 py-2 border rounded-lg bg-gray-100">
                {appointment.hinh_anh_bo_sung && appointment.hinh_anh_bo_sung.length > 0 ? (
                    appointment.hinh_anh_bo_sung.map((file: string, index: number) => {
          ...
          <div className="flex justify-center gap-4 mt-6">
            ...
            >
              ...
              <div className="mt-6 text-center">
                <button className="text-blue-500 underline">Bác sĩ kê đơn</button>
              </div>
            ) : null}
          </div>
      </div>
        ...
        <div className="w-1/3 bg-white p-6 rounded-lg shadow-md">
          <div className="flex flex-col items-center">
            ...
            <p className="text-base font-semibold text-red-600">
              {appointment.benh_nhan_ho_va_ten} ({appointment.benh_nhan_ma_benh_nhan})
            </p>
          ...
          <div className="ml-5">
            {[
              ...
              { label: "Tiền sử bệnh lý", value: appointment.benh_nhan_tien_su_benh},
            ].map((info) => (
