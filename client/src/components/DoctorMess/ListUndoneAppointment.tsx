...
const ListUndoneAppointment: React.FC<Props> = ({ data }) => {
  const navigate = useNavigate();

  if (!data || data.length === 0) return <p>Không có cuộc hẹn nào đang chờ.</p>;

  return (
    <div className="w-full space-y-4">
      <p className="text-sm bg-blue-100 text-blue-800 px-4 py-2 rounded-md italic flex items-center gap-1">
        ⏳ <span>Dưới đây là danh sách các cuộc hẹn đang chờ:</span>
      </p>
...
export default ListUndoneAppointment;
